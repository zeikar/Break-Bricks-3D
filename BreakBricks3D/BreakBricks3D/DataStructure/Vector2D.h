...
//		assert(magnitude > (TT)0);
	...
	void multiplyComponentWise(const TT& xs, const TT& ys)
	{
		x_ *= xs;
		y_ *= ys;
	}

	TT getComponentWiseMax() const
	{
		return MAX2(x_, y_);
	}

	void getComponentWiseMin() const
	{
		return MAX2(x_, y_);
	}

	const TT& operator[](const int& i) const	// returns const unline other *values_
	{
		assert(i >= 0);
		assert(i < 2);		// 2D vector
...
template<class TT> const Vector2D<TT> operator * (const Vector2D<TT>& v, const TT& a)
{
	return Vector2D<TT>(a*v.x_, a*v.y_);
}
